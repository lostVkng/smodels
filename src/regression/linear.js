...
function addConstant(exog, prepend=true){
    ...
    if(prepend){
        exog.unshift({'title':'intercept','data': new Array(exog[0]['data'].length).fill(1), 'constant': true})
    } else{
        exog.push({'title':'intercept', 'data':new Array(exog[0]['data'].length).fill(1), 'constant': true})
    }
...
class Model{
    ...
    */
    get fvalue(){
        ...
        let mseRes = this._data.mseResidual ? this._data.mseResidual : this.mseResidual
    ...
    */
    get fProb(){
        return fdist.probability(this.fvalue, this._data.df.regression, this._data.df.residual)
    }
    ...
    */
    get bic(){
        ...
        return -2 * llf + Math.log(this._data.N) * (this._data.df.regression + this._data.kConstant)
    }
    ...
    */
    get rsq(){
        ...
        let rsq = null
        let ssResidual = this._data.ssResidual ? this._data.ssResidual : this.ssResidual

        if(this._data.kConstant > 0){

            let ssTotalCentered = this._data.ssTotalCentered ? this._data.ssTotalCentered : this.ssTotalCentered

            rsq = Decimal(1).minus( Decimal(ssResidual).dividedBy(ssTotalCentered) ).toNumber()
        } else{

            let ssTotalUncentered = this._data.ssTotalUncentered ? this._data.ssTotalUncentered : this.ssTotalUncentered

            rsq = Decimal(1).minus( Decimal(ssResidual).dividedBy(ssTotalUncentered) ).toNumber()
        }
    ...
    */
    get conditionNumber(){
        ...
        let Asq = this._data.A.reduce((sum, col) => {
            return sum.plus(col.reduce((_s, i) => {
    ...
    */
    get ssRegression(){
        ...
        let ss = Arr.sum(this._data.predicted.map(yh => Decimal(yh).minus(wMean).pow(2).toNumber()))
    ...
    */
    get ssResidual(){
        ...
        let ss = Arr.sum(this._data.wendog.map((y, i) => (Decimal(y).minus(this._data.predicted[i])).pow(2).toNumber()))
    ...
    */
    get ssTotalCentered(){
        ...
        if(this._data.weights){
            ...
            let emWA = this._data.endog.data.map((y, i) => Decimal(y).minus(wa).pow(2).toNumber())

            // sum of weights * emWA
            ss = Arr.sum( emWA.map((v, i) => Decimal(v).times(this._data.weights[i]).toNumber()) )

        } else{
            ss = Arr.sum(this._data.wendog.map((y, i) => Decimal(y).minus(this._data.wendogMean).pow(2).toNumber()))
        }
    ...
    */
    get ssTotalUncentered(){
        ...
        let ss = Arr.sum(this._data.wendog.map(y => Decimal(y).pow(2).toNumber()))
    ...
    */
    get mseRegression(){
        ...
        let ssReg = this._data.ssRegression ? this._data.ssRegression : this.ssRegression
    ...
    */
    fit(){
        ...
        this._data.residuals = this._data.wendog.map((y, i) => Decimal(y).minus(this._data.predicted[i]).toNumber())
        ...
        this._data.exog.map((coeff, i) => {
            coeff.sterror = Decimal(this._data.Ainv[i][i]).times(this.mseResidual).sqrt().toNumber()
            coeff.tstat = Decimal(coeff.beta).dividedBy(coeff.sterror).toNumber()
            coeff.pvalue = Decimal(1).minus(tdist.cdf(Decimal(coeff.tstat).abs().toNumber(), this._data.df.residual)).times(2).toNumber()
    ...
    /*
      @desc Returns an object of the regression results

      @references - summary method from statsmodels/regression/linear.py
      @return {object}
    */
    summaryJson() {
      const json = {};
    ...
    */
    summary(){
        ...
        let A1R = 'R-squared: '
        terminalString += A1R + ' '.repeat(twoColW-A1R.length-this.rsq.toFixed(3).toString().length)+this.rsq.toFixed(3)+'\n'

        let A2L = 'Model: '
        terminalString += A2L + ' '.repeat(twoColW-A2L.length-this.modelType.length)+this.modelType+' '.repeat(4)

        let A2R = 'Adj. R-squared: '
        terminalString += A2R + ' '.repeat(twoColW-A2R.length-this.rsqAdj.toFixed(3).toString().length)+this.rsqAdj.toFixed(3)+'\n'

        let A3L = 'Method: '
        terminalString += A3L + ' '.repeat(twoColW-A3L.length-this.method.length)+this.method+' '.repeat(4)

        let A3R = 'F-statistic: '
        terminalString += A3R + ' '.repeat(twoColW-A3R.length-this.fvalue.toFixed(1).length)+this.fvalue.toFixed(1)+'\n'
        ...
        let A4R = 'Prob (F-stat): '
        terminalString += A4R + ' '.repeat(twoColW-A4R.length-this.fProb.toFixed(6).length)+this.fProb.toFixed(6)+'\n'
        ...
        terminalString += A6L + ' '.repeat(twoColW-A6L.length-this.N.toString().length)+this.N+' '.repeat(4)

        let A6R = 'AIC: '
        terminalString += A6R + ' '.repeat(twoColW-A6R.length-this.aic.toFixed(2).length)+this.aic.toFixed(2)+'\n'

        let A7L = 'Df Residuals: '
        terminalString += A7L + ' '.repeat(twoColW-A7L.length-this._data.df.residual.toString().length)+this._data.df.residual+' '.repeat(4)

        let A7R = 'BIC: '
        terminalString += A7R + ' '.repeat(twoColW-A7R.length-this.bic.toFixed(2).length)+this.bic.toFixed(2)+'\n'

        let A8L = 'Df Regression: '
        terminalString += A8L + ' '.repeat(twoColW-A8L.length-this.K.toString().length)+this.K+'\n'

        let A9L = 'Covariance Type: '
        terminalString += A9L + ' '.repeat(twoColW-A9L.length-this.covtype.length)+this.covtype+'\n'
        ...
        let b2colW = Math.max(...this._data.exog.map(a => a.beta.toFixed(4).length))
        let b3colW = Math.max(...this._data.exog.map(a => a.sterror.toFixed(3).length))
        let b4colW = Math.max(...this._data.exog.map(a => a.tstat.toFixed(3).length))
        let b5colW = Math.max(...this._data.exog.map(a => a.pvalue.toFixed(3).length))
        let b6colW = Math.max(...this._data.exog.map(a => a.ciLower.toFixed(3).length))
        let b7colW = Math.max(...this._data.exog.map(a => a.ciHigher.toFixed(3).length))
        ...
        this._data.exog.forEach((c) => {
            ...
            terminalString += ' '.repeat( Math.floor( (b4colW - c.tstat.toFixed(3).toString().length) )  )+c.tstat.toFixed(3)
            ...
            terminalString += ' '.repeat( Math.floor( (b7colW - c.ciHigher.toFixed(3).toString().length) )  )+c.ciHigher.toFixed(3)
        ...
        let C1L = 'Omnibus: '
        terminalString += C1L + ' '.repeat(twoColW-C1L.length-omnibus.omnibus.toFixed(3).toString().length)+omnibus.omnibus.toFixed(3)+' '.repeat(4)

        let C1R = 'Durbin-Watson: '
        terminalString += C1R + ' '.repeat(twoColW-C1R.length-this.durbinWatson.toFixed(3).toString().length)+this.durbinWatson.toFixed(3)+'\n'

        let C2L = 'Prob(Omnibus): '
        terminalString += C2L + ' '.repeat(twoColW-C2L.length-omnibus.pval.toFixed(3).toString().length)+omnibus.pval.toFixed(3)+' '.repeat(4)

        let jarquebera = this.jarqueBera

        let C2R = 'Jarque-Bera (JB): '
        terminalString += C2R + ' '.repeat(twoColW-C2R.length-jarquebera.jb.toFixed(3).toString().length)+jarquebera.jb.toFixed(3)+'\n'

        let C3L = 'Skew: '
        terminalString += C3L + ' '.repeat(twoColW-C3L.length-this.skew.toFixed(3).toString().length)+this.skew.toFixed(3)+' '.repeat(4)

        let C3R = 'Prob(JB): '
        terminalString += C3R + ' '.repeat(twoColW-C3R.length-jarquebera.pval.toFixed(3).toString().length)+jarquebera.pval.toFixed(3)+'\n'

        let C4L = 'Kurtosis: '
        terminalString += C4L + ' '.repeat(twoColW-C4L.length-this.kurtosis.toFixed(3).toString().length)+this.kurtosis.toFixed(3)+' '.repeat(4)

        let C4R = 'Cond. No. '
        terminalString += C4R + ' '.repeat(twoColW-C4R.length-this.conditionNumber.toFixed(1).toString().length)+this.conditionNumber.toFixed(1)+'\n'
        terminalString += '='.repeat(termWidth) + '\n'
...
class Gls extends Model{
    constructor(endog, exog, {sigma, cholsigmainv} = {} ){
        ...
        if(sigma){
            ...
            let res_fid = new Ols({'title':'a','data':ols_resid.slice(1)}, [{'title':'b','data':ols_resid.slice(0, -1)}]).fit()
    ...
    */
    whiten(X){
        ...
        if(Array.isArray(X[0])){
            ...
            w = X.map((row, i) => row.map(v => Decimal(v).times(Decimal(this._data.weights[i]).sqrt()).toNumber()))

        }else{
            // array
            w = X.map((v, i) => Decimal(v).times(Decimal(this._data.weights[i]).sqrt()).toNumber())
        }
